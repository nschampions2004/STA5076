...
\begin{blockquote}
After checking the normal distributions, we should run an F test to on the variances to make sure they were not unequal.  I used the F test to make sure the test statistic was under the rejection region.  The F test turned up not to be statistically significant.  With the variances checked, we can move forward with testing the means with a t-test.   
\end{blockquote}
...
\indent \indent \textbf{Data Cleaning}
\begin{blockquote}
To begin, we trimmed the data set down to just shootings after 5 PM of each day.  Upon filtering, we sum up each group of inside and outside shootings by date.  After grouping on location, we look at the bar graphs to see what distribution this scenario fits.       
\end{blockquote}
\indent \indent \textbf{Statistical Test}
\begin{blockquote}
...
\indent \indent \textbf{Data Cleaning}
\begin{blockquote}
To arrive at probabilities of Common vs. Aggravated Assaults, we filter the crime down to those two, summed up the total incidents of each group by day and divided them by the total number of Assaults.  Finally, we graphed the two distributions making sure the distributions were normally distributed.  
\end{blockquote}
\indent \indent \textbf{Statistical Test}
\begin{blockquote}
With normally distributed distributions, we take to evaluating if the mean of the probability of falling victim to a common assault was less than that of an aggravated assault.  A t-test to evaluate if that was the case will suffice.  
\end{blockquote}
\indent \textbf{Test 5:} Is there a District with a distinctly higher number of Burglary's in the Summer Months?  If so, which are significantly different?

\indent \indent \textbf{Data Cleaning}
\begin{blockquote}
To begin, we pull out only the Summer months data for Burglaries, grouped by Month/Year, and summarized the total number of incidents.  There was a District equal to 'NA' that was filtered out.  After compiling all the data into a workable form, we graph the residual scatterplot, histogram of residuals, and Normal Q-Q plot.  Seeing nothing remarkable, it is safe to move forward with an ANOVA.
\end{blockquote}
\indent \indent \textbf{Statistical Test}
\begin{blockquote}
